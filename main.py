...
class AdaptiveModularity:
    ...
    def loadDBLP(self):
        if os.path.isfile("./dblp.gpickle"):
            self.G = nx.read_gpickle("./dblp.gpickle")
        else:
            self.G = nx.Graph(gnc = {}, membership = {}, top5000 = {})
            with open("./dblp_raw_data/com-dblp.ungraph.txt", "r") as txt:
                for line in txt:
                    if not line[0] == '#':
                        e = line.split()
                        self.G.add_edge(int(e[0]), int(e[1]))
            with open("./dblp_raw_data/com-dblp.top5000.cmty.txt", "r") as txt:
                count = 0
                for line in txt:
                    if not line[0] == '#':
                        e = line.split()
                        self.G.graph["top5000"][count] = [int(_) for _ in e]   
                        count += 1
            with open("./dblp_raw_data/com-dblp.all.cmty.txt", "r") as txt:
                count = 0
                for line in txt:
                    if not line[0] == '#':
                        e = line.split()
                        ...
                        count += 1
            print "write gpickle file.."
            nx.write_gpickle(self.G,"./dblp.gpickle")

    def loadFootball(self):
        self.G = nx.Graph(gnc = {}, membership = {})
        with open("./football_raw_data/footballTSEinputEL.dat", "r") as f:
            for line in f:
                seg = line.split()
                self.G.add_edge( int(seg[0]), int(seg[1]) )
        with open("./football_raw_data/footballTSEinputConference.clu", "r") as fconf:
...
if __name__ == "__main__":
    ...
    #print "output weighted graph football"
    #algorithm.convert("football.wpairs")

    #print "output weighted graph dblp.."
    #algorithm.loadDBLP()
    #algorithm.convert("dblp.wpairs")
